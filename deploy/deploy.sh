...
REPO_NAME="mf-poc"
IMAGE="${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPO_NAME}/reconcile:latest"
...
gcloud run jobs deploy "${JOB_NAME}" \
  ...
  --memory=1Gi \
  --cpu=1 \
  --set-env-vars="PROJECT_ID=${PROJECT_ID},REGION=${REGION},BUCKET=${BUCKET},DATA_ROOT_PREFIX=${DATA_ROOT_PREFIX},FOLDER_DATE_FORMAT=${FOLDER_DATE_FORMAT},HOT_DAYS=${HOT_DAYS},MAX_WORKERS=20,EXCLUDE_DBS=tmp.db,kafka_test.db,udf.db"
...
echo "========================================================================="
echo "==> 高性能多线程调和引擎部署成功！✔"
echo "    - 镜像已自动打包 reconcile_fast.py (20 线程并发)"
echo "    - 自动排除 tmp.db 等非核心库"
echo "========================================================================="
