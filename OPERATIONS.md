...
### 6.7 `verify.sh`（生产 8 项双向权限断言验证脚本）
...
# 特性：
#   1. 动态定位实际业务表与真实存在的热/冷分区（避免硬编码日期因缺失文件导致 404 误判）
#   2. 前置由写入方 (iceberg-writer) 写入测试实体文件，确保断言检验的是真实 IAM 权限而非 404
#   3. 双向覆盖 Spark/Flink、Hue/分析师、审计冷查询、运维调度 4 类身份安全断言
# ==============================================================================
set -uo pipefail
cd "$(dirname "$0")"
source ./config.env

PROJECT_ID="${PROJECT_ID:-fr-xjsy-prod}"
BUCKET="${BUCKET:-gs://fr-xjsy-bigdata-gcs-dev}"
